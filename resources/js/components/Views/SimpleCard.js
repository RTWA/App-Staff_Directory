...
const SimpleCard = props => {
    ...
    if (onlyMe) {
        return <SimpleTile index={0} key={0} person={me} />;
    }
        ...
        if (selected.id !== "all") {
            Object(person.departments).map(function (dep) {
                if (dep.id === selected.id && !_people.includes(person)) {
                    _people.push(person);
                    _tiles.push(<SimpleTile index={i} key={i} person={person} hod={(person.id == dep.head_id)} />);
                }
                    ...
                    return (
                        Object(selected.children).map(function (child) {
                            if (child.id === dep.id && !_people.includes(person)) {
                                _people.push(person);
                                _tiles.push(<SimpleTile index={i} key={i} person={person} />);
                            }
                        })
                    );
                }
